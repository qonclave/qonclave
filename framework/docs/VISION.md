...
* **Self-Healing Mesh Handoff:** Using the dynamic `/capabilities` API, if a Hub loses power, another Hub instantly detects the drop and takes over listening to the orphaned Edge Cameras. The system remains highly available as long as one Hub survives on the subnet. *The authorization half of this now has a mechanism:* detecting the drop was never the hard part — Hub B has no reason to trust an Edge it did not commission. **Capability grants** (`SECURITY.md` §5) close that gap, and they are verified offline precisely because the Hub that issued them is the one that just died.
...
* **Extreme Low-Power Asynchronous Queues — *specified*:** To support battery-constrained devices (e.g., deep-forest sensors running on coin cells for 5 years), the framework supports asynchronous sleep-queues. Hubs hold commands for days, delivering them only during the brief milliseconds when the Edge device wakes up, rather than requiring an always-on UDP mesh.

  This one has moved from horizon to contract. The mechanism is the **check-in exchange** — one
  request and one response carrying everything in both directions — defined in `COMMUNICATION.md`
  §9 and [`../spec/v1/json-schema/checkin.schema.json`](../spec/v1/json-schema/checkin.schema.json),
  with the device's obligations in [`../spec/v1/profiles/minimal.md`](../spec/v1/profiles/minimal.md).
  The sleep-queue itself is the Hub's per-device **mailbox**: commands held with a TTL, drained on
  contact, retained until acknowledged. Building it surfaced the constraint the bullet above
  implies but does not say out loud — worst-case command latency is one wake interval, so a daily
  sensor can never be told to do something *now*. The C binding implements the device side; the
  Hub-side mailbox is not written yet.
