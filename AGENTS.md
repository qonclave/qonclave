...
- Use `.\examples\hub\setup_hub.ps1` to manage the `geniex-env` virtual environment rather than manually running `pip` for core dependencies.
...
- Do NOT add application-specific business logic or hardcoded rules into the `examples/hub/framework/` files; it must remain use-case agnostic.
- Contain all new features or use-cases entirely within a new directory under `examples/hub/apps/<app_name>/`.
- Strictly subclass `framework.policy.Policy` (or `qonclave.hub.policy.Policy`) and implement the `evaluate` method when building a new application policy.
