...
This is a **reference implementation** consuming the `qonclave` framework via `pip install` (no
internal dependencies on `framework/sdk/python`). Paths in this README are relative to this
directory.
...
- **Broker**: Eclipse Mosquitto, run as its own process — decoupled from
  ...
  mosquitto -c examples\hub\mosquitto.conf -v
  ```
- **Topics**:
