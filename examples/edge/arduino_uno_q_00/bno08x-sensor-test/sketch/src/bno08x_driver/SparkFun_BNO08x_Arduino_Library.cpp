...
boolean BNO08x::reconnectSPI(void)
{
	SPIClass *port = _spiPort;
	const uint8_t cs = _cs;
	const uint8_t intPin = _int_pin;
	const uint8_t resetPin = _reset_pin;
	const uint32_t speed = _spiPortSpeed;

	end();
	delay(100);
	return beginSPI(cs, intPin, resetPin, speed, *port);
}
