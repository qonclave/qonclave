...
def test_successful_fetch_replaces_map_and_hits_the_right_url():
    ...
    def get(url, timeout):
        ...
        return _Response({"people": [{"identity": "priya", "priority": 1},
    ...
    assert c.snapshot() == {"priya": 1, "alice": 2}


def test_malformed_entries_are_skipped_not_fatal():
    payload = {"people": [
        {"identity": "priya", "priority": 1},
...
def test_updated_payload_replaces_map():
    ...
    c = _client()
    with patch("priority_sync.requests.get",
               return_value=_Response({"people": [{"identity": "priya", "priority": 100}]})):
        c.refresh_now()
    with patch("priority_sync.requests.get",
               return_value=_Response({"people": [{"identity": "priya", "priority": 1}]})):
        assert c.refresh_now() is True
